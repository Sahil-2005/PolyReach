...
import { LayoutDashboard, Users, FileText, Send, Settings, Sparkles, ChevronRight, LogOut, X } from 'lucide-react';
...
export function Sidebar({ isOpen, setIsOpen }) {
  return (
    <>
      ...
      <div className={cn(
        "w-72 bg-card/30 backdrop-blur-xl border-r border-white/5 h-screen flex flex-col fixed left-0 top-0 z-50 transition-transform duration-300 md:translate-x-0",
        ...
        <div className="h-20 flex items-center justify-between px-8 border-b border-white/5">
          ...
          <button 
            onClick={() => setIsOpen(false)}
            className="p-2 rounded-lg hover:bg-white/5 md:hidden text-muted-foreground"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
